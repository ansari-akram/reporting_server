...
def multiple_filter(request):
    if request.method == "POST":
        ...
        # wr_answer = Log.objects.filter(event_type_id='3')
        # rt_answer = Log.objects.filter(event_type_id='4')
        # no_answer = Log.objects.filter(event_type_id='5')

        int_event_search = Log.objects.raw('select * from dbo.zayed_university_app_log where intent = "' + intent +
                                           '" and  event_type_id = "' + event_type_id + '"')

        print("======= if =========", int_event_search)
        context = {
            'int_event_obj': int_event_search
        }

        return render('home/advance_filter.html', context)

    else:
        int_event_search = Log.objects.all()
        print("================", int_event_search)
        context = {
            'int_event_obj': int_event_search
        }
